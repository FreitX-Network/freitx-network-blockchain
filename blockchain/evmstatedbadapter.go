...
func (stateDB *EVMStateDBAdapter) CreateAccount(evmAddr common.Address) {
	...
	_, err := stateDB.ws.LoadOrCreateAccountState(addr.OnexAddress(), big.NewInt(0))
	if err != nil {
...
func (stateDB *EVMStateDBAdapter) SubBalance(evmAddr common.Address, amount *big.Int) {
	...
	addr := address.New(stateDB.bc.ChainID(), evmAddr.Bytes())
	state, err := stateDB.ws.CachedAccountState(addr.OnexAddress())
	if err != nil {
...
func (stateDB *EVMStateDBAdapter) AddBalance(evmAddr common.Address, amount *big.Int) {
	...
	addr := address.New(stateDB.bc.ChainID(), evmAddr.Bytes())
	state, err := stateDB.ws.LoadOrCreateAccountState(addr.OnexAddress(), big.NewInt(0))
	if err != nil {
...
func (stateDB *EVMStateDBAdapter) GetNonce(evmAddr common.Address) uint64 {
	addr := address.New(stateDB.bc.ChainID(), evmAddr.Bytes())
	nonce, err := stateDB.ws.Nonce(addr.OnexAddress())
	if err != nil {
...
func (stateDB *EVMStateDBAdapter) Exist(evmAddr common.Address) bool {
	addr := address.New(stateDB.bc.ChainID(), evmAddr.Bytes())
	logger.Debug().Msgf("Check existence of %s", addr.OnexAddress())
	if state, err := stateDB.ws.CachedAccountState(addr.OnexAddress()); err != nil || state == nil {
		logger.Debug().Msgf("account %s does not exist", addr.OnexAddress())
		return false
	}
	return true
}
