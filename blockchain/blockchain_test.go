...
import (
	...
	_hash "github.com/freitx-project/freitx-network-blockchain/pkg/hash"
...
func TestMintDKGBlock(t *testing.T) {
	...
	for i := 0; i < numNodes; i++ {
		...
		addresses[i] = address.New(cfg.Chain.ID, pkHash[:]).OnexAddress()
	...
	for i := 1; i < numNodes; i++ {
		...
		blk, err := chain.MintNewBlock(nil, &iotxAddr,
			&OnexAddress.DKGAddress{PrivateKey: askList[i], PublicKey: pkList[i], ID: idList[i]}, lastSeed, "")
		require.NoError(err)
