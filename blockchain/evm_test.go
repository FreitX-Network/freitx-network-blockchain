...
func TestEVM(t *testing.T) {
	...
	h, _ := OnexAddress.GetPubkeyHash(r.ContractAddress)
...
func TestERC20(t *testing.T) {
	...
	h, _ := OnexAddress.GetPubkeyHash(contract)
	contractAddrHash := byteutil.BytesTo20B(h)
	ws, err = sf.NewWorkingSet()
	require.NoError(err)
	code, err := ws.GetCode(contractAddrHash)
	...
	to, _ := OnexAddress.GetPubkeyHash(ta.Addrinfo["alfa"].RawAddress)
	alfa.SetBytes(to)
	...
	to, _ = OnexAddress.GetPubkeyHash(ta.Addrinfo["bravo"].RawAddress)
	bravo.SetBytes(to)
	value = hash.ZeroHash32B
	h = value[24:]
