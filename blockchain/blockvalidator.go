...
func (v *validator) verifyActions(blk *Block, containCoinbase bool) error {
	...
	for _, tsf := range transfers {
		...
		if !tsf.IsCoinbase() {
			if _, err := OnexAddress.GetPubkeyHash(tsf.Sender()); err != nil {
				return errors.Wrapf(err, "failed to validate transfer sender's address %s", tsf.Sender())
			}
			if _, err := OnexAddress.GetPubkeyHash(tsf.Recipient()); err != nil {
		...
		go func(tsf *action.Transfer, correctTsf *uint64, correctCoinbase *uint64) {
			...
			if tsf.IsCoinbase() {
				...
				if addr.OnexAddress() != tsf.Recipient() {
					return
				}
	...
	for _, vote := range votes {
		...
		if _, err := OnexAddress.GetPubkeyHash(vote.Voter()); err != nil {
			return errors.Wrapf(err, "failed to validate voter's address %s", vote.Voter())
		}
		if vote.Votee() != action.EmptyAddress {
			if _, err := OnexAddress.GetPubkeyHash(vote.Votee()); err != nil {
	...
	for _, execution := range executions {
		...
		if _, err := OnexAddress.GetPubkeyHash(execution.Executor()); err != nil {
		...
		if execution.Contract() != action.EmptyAddress {
			if _, err := OnexAddress.GetPubkeyHash(execution.Contract()); err != nil {
	...
	if blk.SecretWitness != nil {
		...
		if _, err := OnexAddress.GetPubkeyHash(blk.SecretWitness.SrcAddr()); err != nil {
	...
	for _, sp := range blk.SecretProposals {
		// Verify address
		if _, err := OnexAddress.GetPubkeyHash(sp.SrcAddr()); err != nil {
			return errors.Wrapf(err, "failed to validate secret sender's address %s", sp.SrcAddr())
		}
		if _, err := OnexAddress.GetPubkeyHash(sp.DstAddr()); err != nil {
		...
		if v.validatorAddr == sp.DstAddr() {
			validatorID := OnexAddress.CreateID(v.validatorAddr)
