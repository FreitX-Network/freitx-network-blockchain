...
func TestExecutionSignVerify(t *testing.T) {
	...
	executor, err := OnexAddress.NewAddress(OnexAddress.IsTestnet, chainid)
	require.NoError(err)
	contract, err := OnexAddress.NewAddress(OnexAddress.IsTestnet, chainid)
	require.NoError(err)
...
func TestExecutionToJSONFromJSON(t *testing.T) {
	require := require.New(t)
	executor, err := OnexAddress.NewAddress(true, chainid)
	require.NoError(err)
	contract, err := OnexAddress.NewAddress(true, chainid)
	require.NoError(err)
	data, err := hex.DecodeString("60652403")
	require.NoError(err)
