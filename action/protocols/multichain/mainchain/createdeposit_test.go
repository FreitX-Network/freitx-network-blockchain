...
func TestValidateDeposit(t *testing.T) {
	...
	addr, err := address.OnexAddressToAddress(addr1)
	require.NoError(t, err)
	addr2 := address.New(2, addr.Payload()).OnexAddress()
...
func TestMutateDeposit(t *testing.T) {
	...
	assert.Equal(t, address.New(1, subChainAddr[:]).OnexAddress(), receipt.ContractAddress)
}
