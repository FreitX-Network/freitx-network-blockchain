...
func TestProtocol_Handle(t *testing.T) {
	...
	pubKeyHash1, err := OnexAddress.AddressToPKHash(testaddress.Addrinfo["alfa"].RawAddress)
	require.NoError(err)
	pubKeyHash2, err := OnexAddress.AddressToPKHash(testaddress.Addrinfo["bravo"].RawAddress)
	require.NoError(err)
	pubKeyHash3, err := OnexAddress.AddressToPKHash(testaddress.Addrinfo["charlie"].RawAddress)
	require.NoError(err)
	pubKeyHash4, err := OnexAddress.AddressToPKHash(testaddress.Addrinfo["delta"].RawAddress)
	require.NoError(err)
