...
func TestConvertFromAndToOnexAddress(t *testing.T) {
	t.Parallel()

	var chainID [4]byte
	enc.MachineEndian.PutUint32(chainID[:], 1)
	iotxAddr1, err := OnexAddress.NewAddress(OnexAddress.IsTestnet, chainID[:])
	require.NoError(t, err)
	addr, err := V1.OnexAddressToAddress(iotxAddr1.RawAddress)
	require.NoError(t, err)
	iotxAddr2 := addr.OnexAddress()
