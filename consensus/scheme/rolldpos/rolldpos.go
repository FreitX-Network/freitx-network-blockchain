...
import (
	...
	"github.com/freitx-project/freitx-network-blockchain/blocksync"
...
func (ctx *rollDPoSCtx) generateDKGSecrets() ([][]uint32, [][]byte, error) {
	...
	for _, addr := range ctx.epoch.delegates {
		...
		if addr == ctx.addr.RawAddress {
			ctx.epoch.dkgAddress = OnexAddress.DKGAddress{ID: dkgID}
		}
	}
...
type epochCtx struct {
	...
	delegates        []string
	dkgAddress       OnexAddress.DKGAddress
	seed             []byte
}
...
func (b *Builder) SetAddr(addr *OnexAddress.Address) *Builder {
	b.addr = addr
	return b
}
