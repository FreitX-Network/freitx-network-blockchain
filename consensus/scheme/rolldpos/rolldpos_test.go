...
import (
	...
	"github.com/freitx-project/freitx-network-blockchain/test/mock/mock_actpool"
	"github.com/freitx-project/freitx-network-blockchain/test/mock/mock_blockchain"
	"github.com/freitx-project/freitx-network-blockchain/test/mock/mock_explorer"
	"github.com/freitx-project/freitx-network-blockchain/test/mock/mock_network"
...
func TestGenerateDKGSecrets(t *testing.T) {
	...
	assert.Equal(t, OnexAddress.CreateID(ctx.addr.RawAddress), ctx.epoch.dkgAddress.ID)
...
func TestGenerateDKGKeyPair(t *testing.T) {
	...
	for _, addr := range ctx.epoch.delegates {
		dkgID := OnexAddress.CreateID(addr)
...
func TestRollDPoS_convertToConsensusEvt(t *testing.T) {
	...
	vote, err := action.NewVote(2, address.OnexAddress(), address.OnexAddress(), testutil.TestGasLimit, big.NewInt(10))
	require.NoError(t, err)
	var prevHash hash.Hash32B
	blk := blockchain.NewBlock(
		1,
...
func TestUpdateSeed(t *testing.T) {
	...
	for i := 0; i < numNodes; i++ {
		...
		addresses[i] = address.New(chain.ChainID(), pkHash[:]).OnexAddress()
		idList[i] = OnexAddress.CreateID(addresses[i])
...
func TestRollDPoSConsensus(t *testing.T) {
	...
	newConsensusComponents := func(numNodes int) ([]*RollDPoS, []*directOverlay, []blockchain.Blockchain) {
		...
		chainAddrs := make([]*OnexAddress.Address, 0, numNodes)
		...
		addressMap := make(map[string]*OnexAddress.Address, 0)
