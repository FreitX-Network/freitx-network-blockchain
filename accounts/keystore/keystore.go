...
type KeyStore interface {
	...
	Get(string) (*OnexAddress.Address, error)
	Store(string, *OnexAddress.Address) error
	Remove(string) error
	All() ([]string, error)
}
...
type memKeyStore struct {
	accounts map[string]*OnexAddress.Address
}
...
// Get returns OnexAddress from keystore filesystem given raw address
func (ks *plainKeyStore) Get(rawAddr string) (*OnexAddress.Address, error) {
	if err := validateAddress(rawAddr); err != nil {
		return nil, err
	}
	filePath := filepath.Join(ks.directory, rawAddr)
...
// Store stores OnexAddress in keystore filesystem
func (ks *plainKeyStore) Store(rawAddr string, address *OnexAddress.Address) error {
	if err := validateAddress(rawAddr); err != nil {
		return err
	}
	filePath := filepath.Join(ks.directory, rawAddr)
...
// Remove removes the OnexAddress from keystore filesystem given raw address
...
func (ks *plainKeyStore) All() ([]string, error) {
	...
	for _, rawAddr := range names {
		_, err := OnexAddress.GetPubkeyHash(rawAddr)
		if err != nil {
			return nil, err
		}
...
func NewMemKeyStore() KeyStore {
	return &memKeyStore{
		accounts: make(map[string]*OnexAddress.Address),
	}
}
...
// Get returns OnexAddress stored in map given raw address of the account
func (ks *memKeyStore) Get(rawAddr string) (*OnexAddress.Address, error) {
	if err := validateAddress(rawAddr); err != nil {
		return nil, err
	}
...
// Store stores OnexAddress in map
func (ks *memKeyStore) Store(rawAddr string, addr *OnexAddress.Address) error {
	if err := validateAddress(rawAddr); err != nil {
		return err
	}
...
// private functions
//======================================
func keyToAddr(key *Key) (*OnexAddress.Address, error) {
	if key == nil {
	...
	return &OnexAddress.Address{PublicKey: publicKey, PrivateKey: privateKey, RawAddress: key.RawAddress}, nil
}

func validateAddress(addr string) error {
	// check if the address is valid
	_, err := OnexAddress.GetPubkeyHash(addr)
	if err != nil {
