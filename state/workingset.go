...
import (
	...
	"github.com/freitx-project/freitx-network-blockchain/db"
	...
	"github.com/freitx-project/freitx-network-blockchain/trie"
)
...
func (ws *workingSet) LoadOrCreateAccountState(addr string, init *big.Int) (*Account, error) {
	addrHash, err := OnexAddress.AddressToPKHash(addr)
	if err != nil {
		return nil, err
	}
...
func (ws *workingSet) RunActions(
	...
	for addr, state := range ws.cachedStates {
		...
		voteePKHash, err := OnexAddress.AddressToPKHash(account.Votee)
		if err != nil {
			return hash.ZeroHash32B, nil, err
		}
		if addr == voteePKHash {
	...
	for _, e := range executions {
		executorPKHash, err := OnexAddress.AddressToPKHash(e.Executor())
		if err != nil {
			return hash.ZeroHash32B, nil, err
		}
...
func (ws *workingSet) handleVote(producer *Account, blockHeight uint64, votes []*action.Vote, gasLimit *uint64, enableGasCharge bool) error {
	for _, v := range votes {
		...
		voterPKHash, err := OnexAddress.AddressToPKHash(v.Voter())
		if err != nil {
			return err
		}
