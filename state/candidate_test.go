...
func TestCandidate(t *testing.T) {
	...
	pkHash, err := OnexAddress.GetPubkeyHash(cand1.Address)
	require.NoError(err)
	cand1Hash := byteutil.BytesTo20B(pkHash)

	pkHash, err = OnexAddress.GetPubkeyHash(cand2.Address)
	require.NoError(err)
	cand2Hash := byteutil.BytesTo20B(pkHash)

	pkHash, err = OnexAddress.GetPubkeyHash(cand3.Address)
	require.NoError(err)
