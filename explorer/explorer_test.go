...
import (
	...
	"github.com/freitx-project/freitx-network-blockchain/pkg/util/byteutil"
	...
	"github.com/freitx-project/freitx-network-blockchain/test/mock/mock_dispatcher"
	"github.com/freitx-project/freitx-network-blockchain/test/mock/mock_network"
	ta "github.com/freitx-project/freitx-network-blockchain/test/testaddress"
...
func TestService_GetDeposits(t *testing.T) {
	...
	subChainAddr, err := address.OnexAddressToAddress(ta.Addrinfo["producer"].RawAddress)
	require.NoError(err)
	...
	depositAddr1, err := address.OnexAddressToAddress(ta.Addrinfo["alfa"].RawAddress)
	require.NoError(err)
	require.NoError(ws.PutState(
	...
	depositAddr2, err := address.OnexAddressToAddress(ta.Addrinfo["bravo"].RawAddress)
	require.NoError(err)
	require.NoError(ws.PutState(
